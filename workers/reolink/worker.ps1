Param([Parameter(Mandatory=$true)][string] $camera_ip,
        ...
        [Parameter(Mandatory=$true)][int] $day_of_backup,
...
$year = ((Get-Date).AddDays($day_of_backup*(-1))).Year
$month = ((Get-Date).AddDays(($day_of_backup*-1))).Month
$day = ((Get-Date).AddDays(($day_of_backup*-1))).Day
...
if ($token[0] -eq $false)
{
    ...
    Write-Log -Message "Unable to generate token : $($token[1])" -camera_name $name
    Exit 1
}
else
{
    Write-Log -Message "Received the following token $($token[1])" -level "Debug" -camera_name $name
}
...
if ($files[0] -eq $false)
{
    ...
    Write-Log -Message "Unable to get a lit of available files" -camera_name $name
    Exit 2
}
else
{
    Write-Log -Message "Received a list of files " -level "Debug" -camera_name $name
}
...
If(!(test-path $newday_path))
{
    ...
    catch 
    {
        ...
        Write-Log -Message "Error while creating a folder : $_" -camera_name $name
        Exit 3 
    }
}
...
foreach ($file in $files[1])
{
    try
    {
        ...
        Write-Log -Message "About to download file : $($output)" -level "Info" -camera_name $name
    ...
    catch 
    {
        ...
        Write-Log -Message "Error downloading or saving a file : $_" -camera_name $name
        Write-Host $_ 
        Write-Host $name
